...
c
     ...
     *                 wa3,wa4)
      integer, intent(in) :: n
      integer ldfjac,maxfev,mode,nprint,info,nfev,njev,lr
...
c
      ...
      subroutine hybrj2(fcn,n,x,fvec,fjac,ldfjac,xtol,maxfev,diag,mode,
     *     factor,nprint,info,nfev,njev,r,lr,qtf,wa1,wa2,
     *     wa3,wa4)
      integer, intent(in) :: n
      integer ldfjac,maxfev,mode,nprint,info,nfev,njev,lr
...
c     subroutine hybrj2
c
c     hybrj2 is a duplicate of hybrj, intended to be called by ma02aa
c     to get around a non-threadsafe condition encountered
c     in debug mode.
c
c     the purpose of hybrj2 is to find a zero of a system of
c     n nonlinear functions in n variables by a modification
c     of the powell hybrid method. the user must provide a
c     subroutine which calculates the functions and the jacobian.
c
c     the subroutine statement is
c
c       subroutine hybrj2(fcn,n,x,fvec,fjac,ldfjac,xtol,maxfev,diag,
c                        mode,factor,nprint,info,nfev,njev,r,lr,qtf,
c                        wa1,wa2,wa3,wa4)
c
c     where
...
c
c         the value of iflag should not be changed by fcn unless
c         the user wants to terminate execution of hybrj2.
c         in this case set iflag to a negative integer.
c
c       n is a positive integer input variable set to the number
c         of functions and variables.
...
c
      ...
      return
c
c     last card of subroutine hybrj2.
c
      end
...
c
      ...
      subroutine hybrd1(fcn,n,x,fvec,xtol,maxfev,ml,mu,epsfcn,diag,
     *                 mode,factor,nprint,info,nfev,fjac,ldfjac,r,lr,
     *                 qtf,wa1,wa2,wa3,wa4)
      integer n,maxfev,ml,mu,mode,nprint,info,nfev,ldfjac,lr
      double precision xtol,epsfcn,factor
...
c     subroutine hybrd1
c
c			Modified version of hybrd - fcn is called one additional time
c			at exit with iflag=5
c
c     the purpose of hybrd is to find a zero of a system of
c     n nonlinear functions in n variables by a modification
c     of the powell hybrid method. the user must provide a
...
c
      ...
      if (nprint .gt. 0) call fcn(n,x,fvec,iflag)
			...
			! These two lines are the difference with hybrd
			iflag=5
			call fcn(n,x,fvec,iflag)
