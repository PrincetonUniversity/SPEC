...
!write(ounit,*) dAtc(1), dAzc(1)

  ! perturbed boundary condition gB^s
  if (innout .eq. 0) then ! inner surface
    ...
    if (.not. Lcoordinatesingularity) then
      ;                       ; id = Lmh(lvol,1)          ;                           ; rhs(id   ) = -SUM(dAz) / float(Ntz)
    endif 
    ! perturbed toroidal flux
    ;                         ; id = Lmg(lvol,1)          ;                           ; rhs(id   ) = -SUM(dAt) / float(Ntz)
