...
def interface_current(SPEC, interface_number):
    ...
    zeroth components just outside and in.
      Arguments:
        SPEC: An instance of the py_spec.SPEC class run on the output.h5 file.
        interface_number: the number of the interface on which the current is to be calculated.
                          STARTS AT 1!
    """
    import numpy as np
    Ntor = SPEC.physics.Ntor
    # fall out.
...
def interface_current_total(SPEC):
    ...
    """
    import numpy as np
    number_of_interfaces = SPEC.physics.Nvol - 1 # There is one less interface than there are volumes
    current = 0.

    for interface_number in range(1,number_of_interfaces):  #Not doing the last interface (range stops 1 early)
        current += interface_current(SPEC, interface_number)
