...
class SPECNamelist(Namelist):
    ...
    def write(self, filename, force=False):
        ...
        if os.path.exists(filename):
            if not force:
                raise Exception(filename + ' already exists! Please set force to True if you would like to force overwrite.')
    ...
    def remove_volume(self, ivol=0):
        ...
        if ivol==self._Nvol-1 and self['physicslist']['Lfreebound'] == 0:
            raise RuntimeError('You cannot remove the last interface (plasma boundary).')
