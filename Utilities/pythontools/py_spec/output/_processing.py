    ...
    if Igeometry==1 :
        ...
        for j in range(mn):
            if lvol == 0 and im[j] == 0:
                ...
                fac.append(
                    [sbar ** im[j] , (im[j] / 2.0) * sbar ** (im[j] - 1.0)]
                )
            else:
                fac.append([sbar, 0.5 * np.ones(sarr.size)])
...
def _get_zernike(sarr, lrad, mpol):
    ...
    for m in range(mpol+1):
        ...
        for n in range(m+4, lrad+1, 2):
            ...
            dzernike[:, n, m] = factor1 * (2*factor2*r*zernike[:,n-2,m] + (factor2*r**2 - factor3)*dzernike[:,n-2,m] - factor4*dzernike[:,n-4,m])
    ...
    if mpol >= 1:
        for n in range(3, lrad+1, 2):
            zernike[:,n,1] = zernike[:,n,1] - (-1)**((n-1)/2) * float((n+1)/2) * r
            dzernike[:,n,1] = dzernike[:,n,1] - (-1)**((n-1)/2) * float((n+1)/2)
