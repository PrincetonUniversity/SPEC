function Bcontrav = get_spec_magfield_cyl(fdata,lvol,sarr,tarr,zarr)
 
 
% Computes contravariant components of B in volume lvol in cylindrical
% geometry
%
% INPUT
...
%
% OUTPUT
%   -Bcontrav : cell structure with 3 arrays: B^s, B^theta, B^zeta each 
%               with size length(sarr)*length(tarr)*length(zarr)
%
% Note: Stellarator symmetry is only assumed in the Jacobian evaluation
%
% written by A.Baillod (2019)

% get jacobian of the coordinates (stell sym)
jac     = get_spec_jacobian_cyl(fdata,lvol,sarr,tarr,zarr);  
...
T       = cell(Lrad+1,2);  % allocate data for Chebyshev polynomials and
                           % their derivatives
fac     = cell(mn,2);      % allocate data for regularization factors and 
                           % their derivatives
  ...
  for j=1:mn
    for it=1:nt
      for iz=1:nz
       ...
       Bs(:,it,iz) = Bs(:,it,iz) + fac{j}{1}.*T{l}{1}.*( (im(j)*Azo(l,j)...
                     + in(j)*Ato(l,j))*cosa - (im(j)*Aze(l,j) ...
                     + in(j)*Ate(l,j))*sina);
       Bt(:,it,iz) = Bt(:,it,iz) - (fac{j}{1}.*T{l}{2}+fac{j}{2}...
                     .*T{l}{1}).*( Aze(l,j)*cosa + Azo(l,j)*sina );
       Bz(:,it,iz) = Bz(:,it,iz) + (fac{j}{1}.*T{l}{2}+fac{j}{2}...
                     .*T{l}{1}).*( Ate(l,j)*cosa + Ato(l,j)*sina );
      end
    end
  end
end
