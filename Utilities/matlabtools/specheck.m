...
if isequal(fdata.Rbc        , data.input.physics.Rbc                  ) ; disp('ok: fdata.Rbc        ') ; else disp('ERROR: fdata.Rbc        '); matching=0; end
if isequal(fdata.Rbs        , data.input.physics.Rbs                  ) ; disp('ok: fdata.Rbs        ') ; else disp('ERROR: fdata.Rbs        '); matching=0; end

if isequal(fdata.TT         , data.output.TT                          ) ; disp('ok: fdata.TT         ') ; else disp('ERROR: fdata.TT         '); matching=0; end

if isequal(fdata.Vnc        , data.input.physics.Vnc                  ) ; disp('ok: fdata.Vnc        ') ; else disp('ERROR: fdata.Vnc        '); matching=0; end
if isequal(fdata.Vns        , data.input.physics.Vns                  ) ; disp('ok: fdata.Vns        ') ; else disp('ERROR: fdata.Vns        '); matching=0; end
if isequal(fdata.Zbc        , data.input.physics.Zbc                  ) ; disp('ok: fdata.Zbc        ') ; else disp('ERROR: fdata.Zbc        '); matching=0; end
if isequal(fdata.Zbs        , data.input.physics.Zbs                  ) ; disp('ok: fdata.Zbs        ') ; else disp('ERROR: fdata.Zbs        '); matching=0; end
if isequal(fdata.adiabatic  , data.input.physics.adiabatic            ) ; disp('ok: fdata.adiabatic  ') ; else disp('ERROR: fdata.adiabatic  '); matching=0; end
...
if isequal(fdata.dpp        , data.output.dpp                         ) ; disp('ok: fdata.dpp        ') ; else disp('ERROR: fdata.dpp        '); matching=0; end
if isequal(fdata.dqq        , data.output.dqq                         ) ; disp('ok: fdata.dqq        ') ; else disp('ERROR: fdata.dqq        '); matching=0; end
if isequal(fdata.forcetol   , data.output.forcetol                    ) ; disp('ok: fdata.forcetol   ') ; else disp('ERROR: fdata.forcetol   '); matching=0; end

if isequal(fdata.gamma      , data.input.physics.gamma                ) ; disp('ok: fdata.gamma      ') ; else disp('ERROR: fdata.gamma      '); matching=0; end
if isequal(fdata.helicity   , data.input.physics.helicity             ) ; disp('ok: fdata.helicity   ') ; else disp('ERROR: fdata.helicity   '); matching=0; end
...
if isequal(fdata.Ate        , data.output.vector_potential.Ate        ) ; disp('ok: fdata.Ate        ') ; else disp('ERROR: fdata.Ate        '); matching=0; end
if isequal(fdata.Aze        , data.output.vector_potential.Aze        ) ; disp('ok: fdata.Aze        ') ; else disp('ERROR: fdata.Aze        '); matching=0; end
if isequal(fdata.Ato        , data.output.vector_potential.Ato        ) ; disp('ok: fdata.Ato        ') ; else disp('ERROR: fdata.Ato        '); matching=0; end
if isequal(fdata.Azo        , data.output.vector_potential.Azo        ) ; disp('ok: fdata.Azo        ') ; else disp('ERROR: fdata.Azo        '); matching=0; end
