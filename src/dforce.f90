...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
  ...
  REAL,    intent(out) :: force(0:NGdof_field)      ! force;
...
#endif
   dBBdmp(1:LGdof_force,1:Mvol,0:1,1:2) = zero
  endif
...
! ---------------
  ...
  ;   force(0:NGdof_force) = zero

  do vvol = 1, Mvol-1

    LREGION(vvol)

    tdoc = (vvol-1) * LGdof_force
...
#ifdef DEBUG
      FATAL( dforce, idoc.ne.LGdof_force, counting error ) ! this has caught bugs;
#endif
...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
  ...
  if( NGdof_force.ne.0 ) then ; ForceErr = sqrt( sum( force(1:NGdof_force)*force(1:NGdof_force) ) / NGdof_force ) ! this includes spectral constraints;
  else                  ; ForceErr = zero
  endif
...
if( Lhessian3Dallocated .and. Igeometry.ge.3) then ! construct Hessian3D;
  ...
  if (LcomputeDerivatives ) then
      ...
      hessian2D(1:NGdof_force,1:NGdof_bnd) = zero
              ...
              do issym = 0, 1 ! stellarator symmetry;
                ...
                !if( LocalConstraint) then
                   ...
                   if (irz.eq.0) then
                    ...
                    if ( vvol.gt.1 ) then
                       tdofr = (vvol-2) * LGdof_force + idofr ! labels degree-of-freedom in internal interface geometry   ;
                       tdofz = (vvol-2) * LGdof_force + idofz
                       tdoc = (vvol-1) * LGdof_force          ! labels force-balance constraint across internal interfaces; !always fix
                       idoc = 0                         ! local  force-balance constraint across internal interface ;
                       if (irz .eq.0) then
                           hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdofr) = -denergydrr(idoc+1:idoc+LGdof_force ,vvol+0,1,idof,0)
                       else
                           hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdofz+mn_field)= -denergydzr(idoc+1:idoc+LGdof_force,vvol+0,1,idof,0)
                       endif
                     endif ! end of if( vvol.gt.1 ) ;

                        ;tdofr = (vvol-1) * LGdof_force + idofr
                        ;tdofz = (vvol-1) * LGdof_force + idofz
                        ;tdoc = (vvol-1) * LGdof_force ! shorthand;
                        ;idoc = 0 ! diagonal elements;
                       if (irz .eq. 0) then
                          ;hessian2D(tdoc+1:tdoc+LGdof_force,tdofr) = denergydrr(idoc+1:idoc+LGdof_force,vvol+1,0,idof,0) - denergydrr(idoc+1:idoc+LGdof_force,vvol+0,1,idof,1)! &
                          !write(ounit,*) im(ii), hessian2D(1,tdofr) !vvol, im(ii), in(ii), irz, issym, tdofr, tdofz
                        else
                          ;hessian2D(tdoc+1:tdoc+LGdof_force,tdofz+mn_field) = denergydzr(idoc+1:idoc+LGdof_force,vvol+1,0,idof,0) - denergydzr(idoc+1:idoc+LGdof_force,vvol+0,1,idof,1)
                          !write(ounit,*) im(ii), hessian2D(1,tdofr) !vvol, im(ii), in(ii), irz, issym, tdofr, tdofz
                      ...
                      if ( vvol.lt.Mvol-1 ) then
                         tdofr = (vvol+0) * LGdof_force + idofr
                         tdofz = (vvol+0) * LGdof_force + idofz
                         tdoc = (vvol-1) * LGdof_force ! shorthand;
                         idoc = 0
                               !  if    ( im(idof).le.0 ) then ; hessian(tdoc+idof,tdof) = - one
                                  ...
                                  !  endif
                        if (irz.eq.0) then
                             hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdofr) = denergydrr(idoc+1:idoc+LGdof_force ,vvol+1,0,idof,1)
                        else
                             hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdofz+mn_field) = denergydzr(idoc+1:idoc+LGdof_force ,vvol+1,0,idof,1)
                  ...
                  !if( vvol.eq.Mvol-1 ) then
                   ...
                   ! tdoc = (vvol-1) * LGdof_force ! shorthand ;
                    !idoc = 0
                    !dessian3D(tdoc+idoc+1:tdoc+idoc+LGdof_force,idof) = HdFFdRZ(idoc+1:idoc+LGdof_force,0,idof,1,vvol+1)
            ...
            do irz = 0, 1 ! Fourier harmonic of R, Fourier harmonic of Z;
              ...
              do issym = 0, 1 ! stellarator symmetry;
                ...
                if( LocalConstraint) then
                  ...
                  if( vvol.gt.1 ) then
                    tdof = (vvol-2) * LGdof_force + idof ! labels degree-of-freedom in internal interface geometry   ;
                    tdoc = (vvol-1) * LGdof_force        ! labels force-balance constraint across internal interfaces;
                    idoc = 0                       ! local  force-balance constraint across internal interface ;
                    hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdof) =  - HdFFdRZ(idoc+1:idoc+LGdof_force,1,idof,0,vvol+0)
                  ...
                  if( Lextrap.eq.1 .and. vvol.eq.1 ) then
                      ...
                      ;hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdof) = HdFFdRZ(idoc+1:idoc+LGdof_force,0,idof,0,vvol+1) - HdFFdRZ(idoc+1:idoc+LGdof_force,1,idof,1,vvol+0)
                  ...
                  if( vvol.lt.Mvol-1 ) then
                    ...
                    idoc = 0
                    if( Lextrap.eq.1 .and. vvol.eq.1 ) then
                      if    ( im_force(idof).le.0                     ) then ; hessian2D(tdoc+idof,tdof) = - one
                      else                                             ; hessian2D(tdoc+idof,tdof) = - one
                      endif
                    else
                      hessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdof) = HdFFdRZ(idoc+1:idoc+LGdof_force,0,idof,1,vvol+1)
                    endif
                  endif ! end of if( vvol.lt.Mvol-1 ) ;
                  ...
                  if( vvol.eq.Mvol-1 ) then
                    ...
                    tdoc = (vvol-1) * LGdof_force ! shorthand ;
                    idoc = 0
                    dessian2D(tdoc+idoc+1:tdoc+idoc+LGdof_force,idof) = HdFFdRZ(idoc+1:idoc+LGdof_force,0,idof,1,vvol+1)
              ...
              if( LocalConstraint ) then
                ...
                if( Lextrap.eq.1 .and. vvol.eq.1 ) then
                    ...
                    if( Lconstraint.eq.1 ) then ! this is a little clumsy;
                        dforcedRZ(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdof) =  dforcedRZ(tdoc+idoc+1:tdoc+idoc+LGdof_force,tdof)                       &
                                                                    ...
                                                                    - dBBdmp(idoc+1:idoc+LGdof_force,vvol+0,1,2) * dmupfdx(vvol+0,1,2,idof,1)
                    endif ! end of if( Lconstraint.eq.1 );
                endif ! end of if( Lextrap.eq.1 .and. vvol.eq.1 )
                ...
                do ivol = 1, Mvol-1
                  ...
                  dforcedRZ(tdoc+1:tdoc+LGdof_force,tdof) = dforcedRZ(tdoc+1:tdoc+LGdof_force,tdof)                              &
                                                    ...
                                                    - dBBdmp(1:LGdof_force,ivol+0,1,2) * dmupfdx(ivol+0,vvol,2,idof,1)
