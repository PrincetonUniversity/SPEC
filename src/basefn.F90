...
subroutine get_cheby(lss, lrad, cheby)
    ...
    real(wp), intent(inout) :: cheby(0:lrad, 0:1)
    ...
    cheby(1, 0:1) = (/lss, one/)
    do ll = 2, lrad
        cheby(ll, 0:1) = (/two*lss*cheby(ll - 1, 0) - cheby(ll - 2, 0), &
...
subroutine get_cheby_d2(lss, lrad, cheby)
    ...
    real(wp), intent(in) :: lss
    integer, intent(in) :: lrad
    real(wp), intent(inout) :: cheby(0:lrad, 0:2)
...
subroutine get_zernike(r, lrad, mpol, zernike)
    ...
    real(wp), intent(inout) :: zernike(0:lrad, 0:mpol, 0:1)

    real(wp) :: rm, rm1  ! r to the power of m'th and m-1'th
    real(wp) :: factor1, factor2, factor3, factor4
...
subroutine get_zernike_d2(r, lrad, mpol, zernike)
    ...
    real(wp), intent(in) :: r
    integer, intent(in) :: lrad, mpol
    real(wp), intent(inout) :: zernike(0:lrad, 0:mpol, 0:2)

    real(wp) :: rm, rm1, rm2  ! r to the power of m'th, m-1'th and m-2'th
    real(wp) :: factor1, factor2, factor3, factor4
    integer :: m, n  ! Zernike R^m_n
...
subroutine get_zernike_rm(r, lrad, mpol, zernike)
    ...
    real(wp), intent(in) :: r
    integer, intent(in) :: lrad, mpol
    real(wp), intent(inout) :: zernike(0:lrad, 0:mpol)
