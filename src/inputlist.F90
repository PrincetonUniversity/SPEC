...
module inputlist
    ...
    !> The maximum value of \c Nvol is \c MNvol=256.
    integer, parameter :: MNvol = 256

    !> The maximum value of \c Mpol is \c MNpol=64.
    integer, parameter :: MMpol = 128

    !> The maximum value of \c Ntor is \c MNtor=64.
...
!> @{
    ...
    !> stellarator symmetry is enforced if \c Istellsym==1
    integer :: Istellsym = 1
    ...
    !> adiabatic index; cannot set \f$|\gamma| = 1\f$
    real(wp) :: gamma = 0.0
    ...
    !> toroidal flux, \f$\psi_t\f$, enclosed by each interface
    !> <ul>
    ...
    !> <li> \sa phiedge </li>
    !> </ul>
    real(wp) :: tflux(1:MNvol + 1) = 0.0

    !> poloidal flux, \f$\psi_p\f$, enclosed by each interface
    real(wp) :: pflux(1:MNvol + 1) = 0.0

    !> helicity, \f${\cal K}\f$, in each volume, \f${\cal V}_i\f$
    !> <ul>
    !> <li> on exit, \c helicity  is set to the computed values of \f${\cal K} \equiv \int {\bf A}\cdot{\bf B}\;dv\f$ </li>
    !> </ul>
    real(wp) :: helicity(1:MNvol) = 0.0
    ...
    !> </ul>
    real(wp) :: pressure(1:MNvol + 1) = 0.0
    ...
    !> <li> \c pressure is only used in calculation of interface force-balance. </li>
    !> </ul>
    real(wp) :: adiabatic(1:MNvol + 1) = 0.0

    !> helicity-multiplier, \f$\mu\f$, in each volume
    real(wp) :: mu(1:MNvol + 1) = 0.0

    !> Toroidal current constraint normalized by \f$\mu_0\f$ (\f$I_{volume} = \mu_0\cdot [A]\f$), in each volume.
    ...
    real(wp) :: Ivolume(1:MNvol + 1) = 0.0

    !> Toroidal current normalized by \f$\mu_0\f$ at each interface (cumulative). This is the sum of all pressure driven currents.
    real(wp) :: Isurf(1:MNvol) = 0.0

    !> "inside" interface rotational-transform is \f$\mbox{$\,\iota\!\!$-} = (p_l+\gamma p_r)/(q_l+\gamma q_r)\f$,
    ...
    !> rotational-transform, \f$\mbox{$\,\iota\!\!$-}\f$, on inner side of each interface
    !> <ul>
    !> <li> only relevant if illogical input for \c ql and \c qr are provided </li>
    !> </ul>
    real(wp) :: iota(0:MNvol) = 0.0

    !> "outer" interface rotational-transform is \f$\mbox{$\,\iota\!\!$-} = (p_l+\gamma p_r)/(q_l+\gamma q_r)\f$,
    !>  where \f$\gamma\f$ is the golden mean, \f$\gamma = (1 + \sqrt 5 ) / 2\f$.
    !>
    ...
    !> rotational-transform, \f$\mbox{$\,\iota\!\!$-}\f$, on outer side of each interface
    !> <ul>
    !> <li> only relevant if illogical input for \c ql and \c qr are provided </li>
    !> </ul>
    real(wp) :: oita(0:MNvol) = 0.0

    !> accuracy to which \f$\mu\f$ and \f$\Delta\psi_p\f$ are required
    !> <ul>
    ...
    real(wp) :: Rac(0:MNtor) = 0.0

    !>     stellarator symmetric coordinate axis;
    real(wp) :: Zas(0:MNtor) = 0.0

    !> non-stellarator symmetric coordinate axis;
    real(wp) :: Ras(0:MNtor) = 0.0

    !> non-stellarator symmetric coordinate axis;
    real(wp) :: Zac(0:MNtor) = 0.0

    !>     stellarator symmetric boundary components;
    real(wp) :: Rbc(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !>     stellarator symmetric boundary components;
    real(wp) :: Zbs(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !> non-stellarator symmetric boundary components;
    real(wp) :: Rbs(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !> non-stellarator symmetric boundary components;
    real(wp) :: Zbc(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !>     stellarator symmetric boundary components of wall;
    real(wp) :: Rwc(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !>     stellarator symmetric boundary components of wall;
    real(wp) :: Zws(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !> non-stellarator symmetric boundary components of wall;
    real(wp) :: Rws(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !> non-stellarator symmetric boundary components of wall;
    real(wp) :: Zwc(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !>     stellarator symmetric normal field at boundary; vacuum component;
    real(wp) :: Vns(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !>     stellarator symmetric normal field at boundary; plasma component;
    real(wp) :: Bns(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !> non-stellarator symmetric normal field at boundary; vacuum component;
    real(wp) :: Vnc(-MNtor:MNtor, -MMpol:MMpol) = 0.0

    !> non-stellarator symmetric normal field at boundary; plasma component;
    real(wp) :: Bnc(-MNtor:MNtor, -MMpol:MMpol) = 0.0
...
!> @{
    ...
    !> Used to initialize \f$B_{ns}\f$ using an initial fixed-boundary calculation
    !> <ul>
    ...
    !> resolution of the real space grid on which fast Fourier transforms are performed is given by \c Ndiscrete*Mpol*4
    !> <ul>
    ...
    !> the number of \f$n\f$ harmonics used in the Jacobian \f$m=1\f$ harmonic elimination method;
...
!> @{
    ...
    !> the maximum random number of the Beltrami field if \c Linitgues = 3
    real(wp) :: maxrndgues = 1.0
...
!> @{
    ...
    !> controls the weight factor, \c BBweight, in the force-imbalance harmonics
    !> <ul>
...
!> @{
    ...
    !> stands for Poincare plot theta start. Chose at which angle (normalized over \f$\pi\f$) the Poincare field-line tracing start.
    real(wp) :: Ppts = 0.0
    ...
    !> to compute eigenvalues of \f$\nabla {\bf F}\f$
    LOGICAL :: LHevalues = .false.

    !> to compute eigenvectors (and also eigenvalues) of \f$\nabla {\bf F}\f$
    LOGICAL :: LHevectors = .false.

    !> to compute and write to file the elements of \f$\nabla {\bf F}\f$
...
!> @{
    ...
    LOGICAL :: Wwa00aa = .false.
    LOGICAL :: Wpc00ab = .false.
