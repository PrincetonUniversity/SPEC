...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
    ...
    real(wp) :: inRbc(1:mn, 0:Mvol), inZbs(1:mn, 0:Mvol), inRbs(1:mn, 0:Mvol), inZbc(1:mn, 0:Mvol)
    real(wp) :: jRbc(1:mn, 0:Mvol), jZbs(1:mn, 0:Mvol), jRbs(1:mn, 0:Mvol), jZbc(1:mn, 0:Mvol)
    real(wp) :: tmpRbc(1:mn, 0:Mvol), tmpZbs(1:mn, 0:Mvol), tmpRbs(1:mn, 0:Mvol), tmpZbc(1:mn, 0:Mvol) ! use as temp matrices to store iRbc etc

    real(wp) :: jacbase(1:Ntz), jacbasec(1:mn), jacbases(1:mn) ! the 2D Jacobian and its Fourier
    real(wp) :: junkc(1:mn), junks(1:mn) ! these are junk matrices used for fft
    ...
    real(wp), allocatable :: jacrhs(:), djacrhs(:), jacmat(:, :), djacmat(:, :), solution(:), LU(:, :) ! internal matrices used in Jacobian method
    integer, allocatable :: ipiv(:)   ! internal matrices used in  Jacobian method

#ifdef DEBUG
    ! Debug variables
    real(wp) :: dx, threshold ! used to check result with finite difference.
    real(wp) :: newRbc(1:mn, 0:Mvol), newZbs(1:mn, 0:Mvol), newRbs(1:mn, 0:Mvol), newZbc(1:mn, 0:Mvol)
#endif
...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
    ...
    if (ivol .gt. Mvol) then
        write (6, '("rzaxis :      fatal : myid=",i3," ; ivol.gt.Mvol ; perhaps illegal combination Linitialize=2 and Lfreebound=0 ;")') myid
        call MPI_ABORT(MPI_COMM_SPEC, 1, ierr)
        stop "rzaxis : ivol.gt.Mvol : perhaps illegal combination Linitialize=2 and Lfreebound=0  ;"
    end if
    ...
    case (3)
        ...
        if (Lrzaxis .eq. 1) then ! use centroid method
            ...
            if (LComputeDerivatives) then
                ...
                if (.not. allocated(cosi)) then
                    write (6, '("rzaxis :      fatal : myid=",i3," ; .not.allocated(cosi) ; fatal ;")') myid
                    call MPI_ABORT(MPI_COMM_SPEC, 1, ierr)
                    stop "rzaxis : .not.allocated(cosi) : fatal  ;"
                end if

                if (.not. allocated(sini)) then
                    write (6, '("rzaxis :      fatal : myid=",i3," ; .not.allocated(sini) ; fatal ;")') myid
                    call MPI_ABORT(MPI_COMM_SPEC, 1, ierr)
                    stop "rzaxis : .not.allocated(sini) : fatal  ;"
                end if
...
1010        format("rzaxis : ", f10.2, " : myid=", i3, " ; ivol=", i3, " idgetrf idgetrs=", i3, ' ', i3, " ; "a34)
            ...
            if (LComputeDerivatives) then
                ...
                allocate (djacrhs(1:Njac), stat=astat)
                djacrhs(1:Njac) = zero

                allocate (djacmat(1:Njac, 1:Njac), stat=astat)
                djacmat(1:Njac, 1:Njac) = zero
                    ...
                    do irz = 0, 1 ! loop over deformations in R and Z; inside do imn;
                        ...
                        do issym = 0, 1 ! loop over stellarator and non-stellarator symmetric terms;
                            ...
                            if (im(imn) .eq. 0) then ! the jacobian on the RHS does not depend on m=0 terms
                                ...
                                call coords(1, one, Lcurvature, Ntz, mn)
                                cpuo = MPI_WTIME()
                                ! the derivative of Jabobian w.r.t. geometry is computed by coords
                                jacbase = sg(1:Ntz, 1)
                            end if
...
!******* END part used to benchmark the matrices perturbation result with finite difference *******
                ...
                deallocate (djacrhs, stat=astat)

                deallocate (djacmat, stat=astat)
...
subroutine fndiff_rzaxis(Mvol, mn, ivol, jRbc, jRbs, jZbc, JZbs, imn, irz, issym)
    ...
    real(wp), intent(in) :: jRbc(1:mn, 0:Mvol), jZbs(1:mn, 0:Mvol), jRbs(1:mn, 0:Mvol), jZbc(1:mn, 0:Mvol)
