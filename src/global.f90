...
module cputiming
  ...
  REAL    :: TbndRep = 0.0, bndRepT = 0.0
  REAL    :: Twrtend = 0.0, wrtendT = 0.0
...
! read namelists one after another
   ...
   if (Linitialize .le. 0) then
    ...
    if( Lboundary.eq.0 ) then
      ...
      ! TODO: include non stellarator symmetric terms
      ! Format: m, n, bn1, rn1, zn1, rhomn1, bn2, ... 
      allocate(mmRZRZ(1:num_modes), nnRZRZ(1:num_modes), allRZRZ(1:4,1:Nvol,1:num_modes))
...
1015 format("readin : ", 10x ," : Lrad = "257(i2,",",:))
   ...
   FATAL( readin, Lforcereal.lt.0 .or. Lforcereal.gt.1, illegal Lforcereal )
