...
###############################################################################################################################################################
 ...
 ifeq ($(findstring gfortran,$(SPEC_FCOMP)),gfortran)
     ### GNU ###
     FLAGS=-fdefault-real-8 -O2 -fPIC -ffree-line-length-none
 else
     ifeq ($(findstring intel,$(SPEC_FCOMP)),intel)
         ### Intel ###
         FLAGS=-r8 -O2 -ip -no-prec-div -xHost -fPIC
     else
         $(error $$SPEC_FCOMP undefined or invalid)
     endif
 endif
 ...
 netcdfflags=-L$(NETCDF_HOME)/lib -lnetcdf

 EZSPLINECOMPILE=-I$(PSPLINE_HOME)/mod
 EZSPLINELINK=-L$(PSPLINE_HOME)/lib -lpspline -lezcdf -lpthread -lz -lm
...
xspec: $(addsuffix .o,$(allfiles)) $(MACROS) Makefile
	$(FC) $(FLAGS) $(DFLAGS) -p -o xspec $(addsuffix .o,$(allfiles)) $(nagflags) $(EZSPLINELINK) $(HDF5COMPILE) $(HDF5LINK) $(netcdfflags) $(EZSPLINECOMPILE)
	date
	/bin/echo -e "\a"
