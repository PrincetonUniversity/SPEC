...
!latex \item The normal field at the computational boundary, $\partial {\cal D}$, should be equal to $\left({\bf B}_P + {\bf B}_C\right)\cdot {\bf n}$,
...
!latex \subsection{construction of normal field} 

!latex \begin{enumerate}
!latex \item The normal vector to the computational domain is given as follows:
!latex       \bi
!latex       \item[] \inputvar{Igeometry.eq.1} : Cartesian \\
!latex       $\bx = \t \; \hat i + \z \; \hat j + R(\t,\z) \; \hat k$ \\
!latex       ${\bf e}_\t \times {\bf e}_\z = - R_\t \; \hat i - R_\z \; \hat j + \hat k$
!latex       \item[] \inputvar{Igeometry.eq.2} : Cylindrical \\
!latex       \item[] \inputvar{Igeometry.eq.3} : Toroidal \\
!latex       $\bx = R(\t,\z) \cos \z \; \hat i + R(\t,\z) \sin \z \; \hat j + Z(\t,\z) \; \hat k$ \\
...
!latex \begin{enumerate}
!latex \item The computational boundary is obtained using \link{coords}.
!latex       (Note that the computational boundary does not change, so this needs only to be determined once.)
!latex \item At each point on the computational boundary (i.e., on the discrete grid), 
!latex       \link{casing} is used to compute the plasma field using the virtual casing principle.
!latex \item In toroidal geometry, the vector transformation from Cartesian to cylindrical is given by
!latex       \be \begin{array}{cccccccccccccccccccccc}
!latex           B^R    & = &   & + B_x \cos \z & + & B_y \sin \z &       & \\
!latex           B^\phi & = & ( & - B_x \sin \z & + & B_y \cos \z & ) / R & \\
!latex           B^Z    & = &   &               &   &             &       & B_z
!latex           \end{array}
!latex       \ee
!latex \end{enumerate}
...
!latex The surface integral is performed using \nag{www.nag.co.uk/numeric/FL/manual19/pdf/D01/d01daf_fl19.pdf}{D01DAF}.
    
!   tetalow = tetazeta(1) - vcasingper * pi ; tetaupp = tetazeta(1) + vcasingper * pi ; absacc = vcasingtol 
    tetalow =             - vcasingper * pi ; tetaupp =             + vcasingper * pi ; absacc = vcasingtol 
...
REAL function vcintegrand( lteta, lzeta ) ! differential virtual-casing field; format is fixed by NAG requirements;
